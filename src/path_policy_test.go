...
func TestPathPolicyEngine_Basic(t *testing.T) {
	...
	if len(e.ListRules()) < 8 {
		t.Errorf("expected >= 8 default rules, got %d", len(e.ListRules()))
	}
	...
	if len(ctx.Steps) != 1 {
		t.Errorf("steps=%d", len(ctx.Steps))
	}
}
...
func TestPathPolicyEngine_SequenceRule(t *testing.T) {
	...
	if d.RuleID != "pp-001" {
		t.Errorf("expected pp-001, got %s", d.RuleID)
	}
}
...
func TestPathPolicyEngine_SequenceRuleExpired(t *testing.T) {
	...
	d := e.Evaluate(traceID, "send_email")
	if d.Decision != "allow" {
		t.Errorf("expected allow (expired window), got %s", d.Decision)
	}
}
...
func TestPathPolicyEngine_CumulativeRule(t *testing.T) {
	...
	if d.Decision != "block" {
		t.Errorf("expected block (PII >= 3), got %s", d.Decision)
	}
	if d.RuleID != "pp-004" {
		t.Errorf("expected pp-004, got %s", d.RuleID)
	}
}
...
func TestPathPolicyEngine_DegradationRule(t *testing.T) {
	...
	d := e.Evaluate(traceID, "any_action")
	if d.Decision != "block" {
		t.Errorf("expected block (risk > 80), got %s (%s)", d.Decision, d.Reason)
	}
}
...
func TestPathPolicyEngine_RiskScoreDecay(t *testing.T) {
	...
	if initialScore < 25 {
		t.Errorf("expected score >= 25, got %.1f", initialScore)
	}
	time.Sleep(2 * time.Second) // wait for decay
	ctx2 := e.GetContext(traceID)
	if ctx2.RiskScore >= initialScore {
		t.Errorf("score should have decayed: %.1f >= %.1f", ctx2.RiskScore, initialScore)
	}
}
...
func TestPathPolicyEngine_RiskScoreThresholds(t *testing.T) {
	...
	if d1.Decision != "warn" && d1.Decision != "block" {
		t.Errorf("expected warn or block at score 65, got %s", d1.Decision)
	}
	...
	if d2.Decision != "block" {
		t.Errorf("expected block at score 85, got %s", d2.Decision)
	}
	...
	if d3.Decision != "isolate" && d3.Decision != "block" {
		t.Errorf("expected isolate/block at score 96, got %s", d3.Decision)
	}
}
...
func TestPathPolicyEngine_MultipleRules(t *testing.T) {
	...
	if d.Decision != "block" && d.Decision != "isolate" {
		t.Errorf("expected block/isolate, got %s", d.Decision)
	}
}
...
func TestPathPolicyEngine_RuleCRUD(t *testing.T) {
	...
	if len(e.ListRules()) != initial+1 {
		t.Errorf("expected %d rules, got %d", initial+1, len(e.ListRules()))
	}
	...
	if len(e.ListRules()) != initial {
		t.Errorf("expected %d, got %d", initial, len(e.ListRules()))
	}
...
func TestPathPolicyEngine_SessionIntegration(t *testing.T) {
	...
	if ctx.SessionID != "sess-123" {
		t.Errorf("session_id=%s", ctx.SessionID)
	}
}
...
func TestPathPolicyEngine_TaintIntegration(t *testing.T) {
	...
	ctx := e.GetContext(traceID)
	if len(ctx.TaintLabels) != 1 {
		t.Errorf("taint_labels=%d", len(ctx.TaintLabels))
	}
	if ctx.TaintLabels[0] != "PII-TAINTED" {
		t.Errorf("label=%s", ctx.TaintLabels[0])
	}
}
...
func TestPathPolicyEngine_RuleDisable(t *testing.T) {
	...
	e.RegisterStep(traceID, PathStep{Stage: "tool_call", Action: "web_fetch", Timestamp: time.Now()})
	d := e.Evaluate(traceID, "send_email")
	if d.Decision == "block" && d.RuleID == "pp-001" {
...
func TestPathPolicyEngine_Tenant(t *testing.T) {
	...
	if len(tenantA) != 1 {
		t.Errorf("expected 1 tenant-a rule, got %d", len(tenantA))
	}
	if len(all) <= len(tenantA) {
		t.Error("all rules should be more than tenant-a rules")
	}
	...
	if ctx.TenantID != "tenant-a" {
		t.Errorf("tenant=%s", ctx.TenantID)
	}
}
...
func TestPathPolicyEngine_Concurrency(t *testing.T) {
	...
	// no panic = pass
	if e.ContextCount() == 0 {
		t.Error("expected some contexts")
	}
}
...
func TestPathPolicyEngine_Eviction(t *testing.T) {
	...
	if e.GetContext("evict-1") != nil {
		t.Error("context should have been evicted")
	}
}
...
func TestPathPolicyEngine_Stats(t *testing.T) {
	...
	if stats.ActiveContexts != 2 {
		t.Errorf("active=%d", stats.ActiveContexts)
	}
	if stats.TotalRules < 8 {
		t.Errorf("rules=%d", stats.TotalRules)
	}
	if stats.EnabledRules < 8 {
		t.Errorf("enabled=%d", stats.EnabledRules)
	}
}
...
func TestPathPolicyEngine_EmptyPath(t *testing.T) {
	...
	if e.ContextCount() != 0 {
		t.Error("empty trace should not create context")
	}
}
...
func TestPathPolicyEngine_RiskWeights(t *testing.T) {
	...
	if w != 30 {
		t.Errorf("expected 30, got %.1f", w)
	}

	// Custom weight
	e.SetRiskWeight("custom_action", 99)
	if e.GetRiskWeight("custom_action") != 99 {
		t.Error("custom weight not set")
	}
	...
	if ctx.RiskScore < 90 {
		t.Errorf("expected score >= 90, got %.1f", ctx.RiskScore)
	}
}
...
func TestPathPolicyEngine_DBPersistence(t *testing.T) {
	...
	if count2 != count1 {
		t.Errorf("persistence: expected %d rules, got %d", count1, count2)
	}
	r := e2.GetRule("persist-1")
	if r == nil {
		t.Fatal("persisted rule not found")
	}
	if r.Name != "persistent_rule" {
		t.Errorf("name=%s", r.Name)
	}
}
...
func TestPathPolicyEngine_EventLogging(t *testing.T) {
	...
	if events[0]["trace_id"] != traceID {
		t.Errorf("trace_id=%v", events[0]["trace_id"])
	}
	if events[0]["decision"] != "block" {
		t.Errorf("decision=%v", events[0]["decision"])
	}
}
...
func TestPathPolicyEngine_CredentialCumulative(t *testing.T) {
	...
	if d.Decision != "block" {
		t.Errorf("expected block for credential, got %s", d.Decision)
	}
	if d.RuleID != "pp-005" {
		t.Errorf("expected pp-005, got %s", d.RuleID)
	}
}
...
func TestPathPolicyEngine_ToolHistory(t *testing.T) {
	...
	ctx := e.GetContext(traceID)
	if len(ctx.ToolHistory) != 2 {
		t.Errorf("expected 2 tools, got %d", len(ctx.ToolHistory))
	}
}
...
func TestPathPolicyEngine_JSONConditions(t *testing.T) {
	...
	if cc.Label != "PII-TAINTED" || cc.Threshold != 3 {
		t.Errorf("cumulative: %+v", cc)
	}
	var dc DegradationCondition
	json.Unmarshal([]byte(`{"risk_threshold":60,"degrade_to":"warn"}`), &dc)
	if dc.RiskThreshold != 60 || dc.DegradeTo != "warn" {
		t.Errorf("degradation: %+v", dc)
	}
}
...
func TestPathPolicyEngine_ShellSequence(t *testing.T) {
	...
	if d.Decision != "block" {
		t.Errorf("expected block (pp-002), got %s", d.Decision)
	}
}
