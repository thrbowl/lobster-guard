...
func (e *PathPolicyEngine) loadRules() {
	...
	for rows.Next() {
		...
		var en int
		if rows.Scan(&r.ID, &r.Name, &r.RuleType, &r.Conditions, &r.Action, &en, &r.Priority, &r.Description, &r.TenantID, &r.CreatedAt, &r.UpdatedAt) != nil {
			continue
		}
		r.Enabled = en != 0
	...
	if len(rules) == 0 {
		rules = append([]PathPolicyRule{}, defaultPathPolicies...)
	}
	e.rules = rules
}
...
func (e *PathPolicyEngine) RegisterStep(traceID string, step PathStep) {
	...
	if step.RiskDelta == 0 {
		if w, ok := e.riskWeights[step.Action]; ok {
			step.RiskDelta = w
		}
	}
	ctx.Steps = append(ctx.Steps, step)
	ctx.LastUpdated = time.Now()
	if step.Stage == "tool_call" && step.Action != "" {
		ctx.ToolHistory = append(ctx.ToolHistory, step.Action)
	}
...
func (e *PathPolicyEngine) SetSessionID(traceID, sid string) {
	e.mu.Lock()
	defer e.mu.Unlock()
	if ctx, ok := e.contexts[traceID]; ok {
		ctx.SessionID = sid
	}
}
...
func (e *PathPolicyEngine) ListContexts() []PathContext {
	...
	for _, ctx := range e.contexts {
		c := *ctx
		c.RiskScore = e.decayScore(ctx)
		r = append(r, c)
	}
	return r
}
func (e *PathPolicyEngine) ContextCount() int {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
// v23.1: 攻击者画像联动
func (e *PathPolicyEngine) SetUserProfileEngine(upe *UserProfileEngine) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.userProfileEng = upe
}
...
func (e *PathPolicyEngine) userPriorScore(senderID string) float64 {
	if e.userProfileEng == nil || senderID == "" {
		return 0
	}
...
func (e *PathPolicyEngine) SetHalfLife(sec float64) {
	e.mu.Lock()
	defer e.mu.Unlock()
	if sec > 0 {
		e.halfLifeSec = sec
...
func (e *PathPolicyEngine) SetRiskWeight(a string, w float64) {
	e.mu.Lock()
	defer e.mu.Unlock()
	e.riskWeights[a] = w
}
func (e *PathPolicyEngine) GetRiskWeight(a string) float64 {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.riskWeights[a]
}
...
func (e *PathPolicyEngine) Evaluate(traceID, proposed string) PathDecision {
	...
	for _, rule := range rules {
		...
		if rule.TenantID != "" && rule.TenantID != "default" && rule.TenantID != tenantID {
			continue
		}
		var d *PathDecision
		switch rule.RuleType {
		case "sequence":
			d = e.evalSeq(ctx, rule, proposed)
		case "cumulative":
			d = e.evalCum(ctx, rule)
		case "degradation":
			d = e.evalDeg(ctx, rule, score)
		}
		if d != nil {
			d.RiskScore = score
			if best == nil || actionSev(d.Decision) > actionSev(best.Decision) {
				best = d
			}
		}
	}
	if best != nil {
		e.logEvt(traceID, ctx, best)
		return *best
	}
	return def
}
...
func (e *PathPolicyEngine) evalSeq(ctx *PathContext, rule PathPolicyRule, proposed string) *PathDecision {
	...
	if json.Unmarshal([]byte(rule.Conditions), &c) != nil || proposed != c.Before {
		return nil
	}
	now := time.Now()
	w := time.Duration(c.WindowSec) * time.Second
...
func (e *PathPolicyEngine) evalCum(ctx *PathContext, rule PathPolicyRule) *PathDecision {
	...
	if json.Unmarshal([]byte(rule.Conditions), &c) != nil {
		return nil
	}
	n := 0
	for _, l := range ctx.TaintLabels {
...
func (e *PathPolicyEngine) QueryEvents(traceID, since, tenantID string, limit int) ([]map[string]interface{}, error) {
	...
	for rows.Next() {
		var id int64
		var ti, si, ri, rn, dc, re, tn, ca string
		var rs float64
		var pl int
		if rows.Scan(&id, &ti, &si, &ri, &rn, &rs, &dc, &re, &pl, &tn, &ca) != nil {
			continue
		}
...
func (e *PathPolicyEngine) ListRules() []PathPolicyRule {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return append([]PathPolicyRule{}, e.rules...)
}
...
func (e *PathPolicyEngine) AddRule(rule PathPolicyRule) error {
	...
	if rule.RuleType == "" {
		return fmt.Errorf("rule_type required")
	}
	if rule.TenantID == "" {
	...
	for _, r := range e.rules {
		if r.ID == rule.ID {
			return fmt.Errorf("rule %q exists", rule.ID)
		}
	}
	if e.db != nil {
		if _, err := e.db.Exec("INSERT INTO path_policies (id,name,rule_type,conditions,action,enabled,priority,description,tenant_id) VALUES (?,?,?,?,?,?,?,?,?)",
			rule.ID, rule.Name, rule.RuleType, rule.Conditions, rule.Action, boolToInt(rule.Enabled), rule.Priority, rule.Description, rule.TenantID); err != nil {
			return err
		}
	}
...
func (e *PathPolicyEngine) UpdateRule(rule PathPolicyRule) error {
	...
	if rule.Name != "" {
		e.rules[idx].Name = rule.Name
	}
	if rule.RuleType != "" {
		e.rules[idx].RuleType = rule.RuleType
	}
	if rule.Conditions != "" {
		e.rules[idx].Conditions = rule.Conditions
	}
	if rule.Action != "" {
		e.rules[idx].Action = rule.Action
	}
	e.rules[idx].Enabled = rule.Enabled
	if rule.Priority > 0 {
		e.rules[idx].Priority = rule.Priority
	}
	if rule.Description != "" {
		e.rules[idx].Description = rule.Description
	}
	if e.db != nil {
...
func (e *PathPolicyEngine) DeleteRule(id string) error {
	e.mu.Lock()
	defer e.mu.Unlock()
	n := make([]PathPolicyRule, 0, len(e.rules))
	found := false
	for _, r := range e.rules {
		if r.ID == id {
			found = true
	...
	e.rules = n
	if e.db != nil {
		e.db.Exec("DELETE FROM path_policies WHERE id=?", id)
	}
	return nil
}
...
func (e *PathPolicyEngine) SetRuleEnabled(id string, en bool) error {
	...
	for i, r := range e.rules {
		if r.ID == id {
			...
			if e.db != nil {
				e.db.Exec("UPDATE path_policies SET enabled=?,updated_at=CURRENT_TIMESTAMP WHERE id=?", boolToInt(en), id)
			}
			return nil
		}
	}
...
func (e *PathPolicyEngine) evictionLoop() {
	...
	for range t.C {
		...
		for k, ctx := range e.contexts {
			if now.Sub(ctx.LastUpdated) > e.evictAfter {
				delete(e.contexts, k)
			}
		}
		e.mu.Unlock()
...
func (e *PathPolicyEngine) UpdateTemplate(t PolicyTemplate) error {
	...
	if t.Name != "" {
		e.templates[idx].Name = t.Name
	}
	if t.Description != "" {
		e.templates[idx].Description = t.Description
	}
	if t.Category != "" {
		e.templates[idx].Category = t.Category
	}
	if t.RuleIDs != nil {
		e.templates[idx].RuleIDs = t.RuleIDs
	}
...
func (e *PathPolicyEngine) DeleteTemplate(id string) error {
	...
	e.templates = n
	if e.db != nil {
		e.db.Exec("DELETE FROM policy_templates WHERE id=? AND built_in=0", id)
	}
	return nil
}
...
func (e *PathPolicyEngine) SetTemplateEnabled(id string, en bool) error {
	...
	for i, t := range e.templates {
		if t.ID == id {
			...
			if e.db != nil {
				e.db.Exec("UPDATE policy_templates SET enabled=?,updated_at=CURRENT_TIMESTAMP WHERE id=?", boolToInt(en), id)
			}
			return nil
		}
	}
...
func (e *PathPolicyEngine) ActivateTemplate(id string) (int, error) {
	t := e.GetTemplate(id)
	if t == nil {
		return 0, fmt.Errorf("template %q not found", id)
	}
	activated := 0
	for _, rid := range t.RuleIDs {
		if err := e.SetRuleEnabled(rid, true); err == nil {
			activated++
		}
	}
...
func (e *PathPolicyEngine) DeactivateTemplate(id string) (int, error) {
	...
	deactivated := 0
	for _, rid := range t.RuleIDs {
		if err := e.SetRuleEnabled(rid, false); err == nil {
			deactivated++
		}
	}
