...
func NewSemanticDetector(db *sql.DB, cfg SemanticConfig) *SemanticDetector {
	if cfg.Threshold <= 0 {
		cfg.Threshold = 0.35
	} // BUG-006 fix: lower default threshold from 0.7 to 0.35
	if cfg.Action == "" {
		cfg.Action = "warn"
	}
	if cfg.TFIDFWeight <= 0 {
		cfg.TFIDFWeight = 0.3
	}
	if cfg.SyntaxWeight <= 0 {
		cfg.SyntaxWeight = 0.3
	}
	if cfg.AnomalyWeight <= 0 {
		cfg.AnomalyWeight = 0.2
	}
	if cfg.IntentWeight <= 0 {
		cfg.IntentWeight = 0.2
	}
	...
	sd.buildIDF()
	for i := range sd.attackVectors {
		sd.attackVectors[i].Vector = sd.computeTFIDF(sd.attackVectors[i].Text)
	}
...
func (sd *SemanticDetector) loadAttackPatterns() {
	...
	for _, r := range raw {
		sd.attackVectors = append(sd.attackVectors, attackVector{ID: r.id, Category: r.cat, Text: r.text})
	}
}
...
func (sd *SemanticDetector) buildIDF() {
	N := float64(len(sd.attackVectors))
	if N == 0 {
		N = 1
	}
...
func (sd *SemanticDetector) Analyze(text string) *SemanticResult {
	sd.mu.RLock()
	cfg := sd.config
	avs := sd.attackVectors
	sd.mu.RUnlock()
	details := map[string]float64{}
	tfidf, pid, pcat := sd.analyzeTFIDF(text, avs)
	details["tfidf_max_similarity"] = tfidf
	syntax, sH := sd.analyzeSyntax(text)
	details["syntax_command"] = sH["command"]
	details["syntax_roleplay"] = sH["roleplay"]
	details["syntax_separator"] = sH["separator"]
	anomaly, aH := sd.analyzeAnomaly(text)
	details["anomaly_mixed_script"] = aH["mixed_script"]
	details["anomaly_base64"] = aH["base64"]
	details["anomaly_zero_width"] = aH["zero_width"]
	details["anomaly_entropy"] = aH["entropy"]
	intent, iH := sd.analyzeIntent(text)
	details["intent_dangerous"] = iH["dangerous"]
	details["intent_role_switch"] = iH["role_switch"]
	details["intent_data_extract"] = iH["data_extract"]
	...
	var expl []string
	if tfidf > 0.3 {
		expl = append(expl, fmt.Sprintf("TF-IDF相似(%.2f,category=%s)", tfidf, pcat))
	}
	if syntax > 0.3 {
		expl = append(expl, fmt.Sprintf("句法特征(%.2f)", syntax))
	}
	if anomaly > 0.3 {
		expl = append(expl, fmt.Sprintf("文本异常(%.2f)", anomaly))
	}
	if intent > 0.3 {
		expl = append(expl, fmt.Sprintf("危险意图(%.2f)", intent))
	}
	explanation := "normal"
	if len(expl) > 0 {
	...
	atomic.AddInt64(&sd.totalAnalyzes, 1)
	atomic.AddInt64(&sd.totalScoreSum, int64(score*100))
	if action == "block" {
		atomic.AddInt64(&sd.totalBlocks, 1)
	} else if action == "warn" {
		atomic.AddInt64(&sd.totalWarns, 1)
	}
	return r
}

func (sd *SemanticDetector) analyzeTFIDF(text string, avs []attackVector) (float64, string, string) {
	iv := sd.computeTFIDF(text)
	if len(iv) == 0 {
		return 0, "", ""
	}
	var best float64
...
func (sd *SemanticDetector) analyzeSyntax(text string) (float64, map[string]float64) {
	...
	for _, re := range sd.commandPats {
		if re.MatchString(text) {
			h["command"] = 1.0
			break
		}
	}
	for _, re := range sd.rolePlayPats {
		if re.MatchString(text) {
			h["roleplay"] = 1.0
			break
		}
	}
	for _, re := range sd.separatorPats {
		if re.MatchString(text) {
			h["separator"] = 1.0
			break
		}
...
func (sd *SemanticDetector) analyzeAnomaly(text string) (float64, map[string]float64) {
	...
	if semDetectMixedScripts(text) {
		h["mixed_script"] = 1.0
	}
	if semDetectBase64(text) {
		h["base64"] = 1.0
	}
	h["zero_width"] = semDetectZeroWidth(text)
	if ent := semCalcEntropy(text); len(text) > 20 {
		if ent > 5.5 {
			h["entropy"] = math.Min((ent-5.5)/2.0, 1.0)
		} else if ent < 2.0 && ent > 0 {
			h["entropy"] = math.Min((2.0-ent)/2.0, 1.0)
		}
	}
...
func (sd *SemanticDetector) analyzeIntent(text string) (float64, map[string]float64) {
	...
	for _, ip := range sd.intentPats {
		if ip.re.MatchString(text) {
			if ip.weight > h[ip.category] {
				h[ip.category] = ip.weight
			}
		}
	}
	n := 0
...
func semDetectBase64(text string) bool {
	for _, m := range semBase64RE.FindAllString(text, 5) {
		if d, e := base64.StdEncoding.DecodeString(m); e == nil && len(d) > 10 && utf8.Valid(d) {
			return true
		}
		if d, e := base64.RawStdEncoding.DecodeString(m); e == nil && len(d) > 10 && utf8.Valid(d) {
			return true
		}
	}
	return false
...
func (sd *SemanticDetector) GetConfig() SemanticConfig {
	sd.mu.RLock()
	defer sd.mu.RUnlock()
	return sd.config
}
...
func (sd *SemanticDetector) UpdateConfig(cfg SemanticConfig) {
	...
	defer sd.mu.Unlock()
	if cfg.Threshold > 0 {
		sd.config.Threshold = cfg.Threshold
	}
	if cfg.Action != "" {
		sd.config.Action = cfg.Action
	}
	if cfg.TFIDFWeight > 0 {
		sd.config.TFIDFWeight = cfg.TFIDFWeight
	}
	if cfg.SyntaxWeight > 0 {
		sd.config.SyntaxWeight = cfg.SyntaxWeight
	}
	if cfg.AnomalyWeight > 0 {
		sd.config.AnomalyWeight = cfg.AnomalyWeight
	}
	if cfg.IntentWeight > 0 {
		sd.config.IntentWeight = cfg.IntentWeight
	}
}
...
func (sd *SemanticDetector) Stats() map[string]interface{} {
	...
	avg := 0.0
	if total > 0 {
		avg = float64(scoreSum) / float64(total) / 100.0
	}
	sd.mu.RLock()
	patCount := len(sd.attackVectors)
	sd.mu.RUnlock()
